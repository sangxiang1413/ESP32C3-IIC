...
void oled_run(void)
{
	...
	OLED_P6x8Str(0,2,str1);
	OLED_P6x8Str(0,5,str2);
	...
	while(1)
	{
		...
		sprintf(&info_buf[tem_pos], "%2.2f", temperature);
		//for(int i = 0; i < 10; i++)	printf("%c", info_buf[i]);
		OLED_P6x8Str(0, 4, (unsigned char *)&info_buf[0]);
		
		memset(info_buf, 0x20, 20);
		sprintf(&info_buf[hum_pos], "%2.2f", humidness);
		//for(int i = 0; i < 10; i++)	printf("%c", info_buf[i]);
		OLED_P6x8Str(0, 5, (unsigned char *)&info_buf[0]);
